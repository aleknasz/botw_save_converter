...
namespace Core
{
    public enum SaveType : uint {
        ...
        // 0x1E470000 or 0x1B470000
        WiiU = 0x1E470000,
        WiiU_B = 0x1B470000,
        
        // 0x0000471E or 0x0000471B
        Switch = 0x0000471E,
    ...
    public class BotwSave
    {
            ...
            foreach (var file in Directory.EnumerateFiles(SourceFolder, "game_data.sav", SearchOption.AllDirectories)) {
                ...
                try {
                    ...
                    using (FileStream fs = File.OpenRead(file)) {
                        ...
                        if (SaveType == SaveType.WiiU || SaveType == SaveType.WiiU_B) {
                        ...
                        Console.WriteLine($"Found {SaveType.ConsoleName()} version {version} on \"{file}\"");
                    }
                }
                catch (Exception ex) {
                    Console.WriteLine($"Could not identify version on save '{Path.GetFileName(Path.GetDirectoryName(file))}' | {ex}");
                    if (!skipVersionCheck) {
                        throw;
                    }
                }
        ...
        {
            if (dst == SourceFolder) {
                Console.WriteLine($"Backing up source directory...");
                DirectoryHelper.Copy(SourceFolder, $"{SourceFolder}/backup", true);
            }
            else {
                Console.WriteLine($"Copying save files to the output directory...");
